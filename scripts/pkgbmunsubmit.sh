...
for opt in $BYTEMAN_JAVA_OPTS; do
	label=`echo $opt | awk '{split($0,tab,"=");print tab[1]}'`
	${JAVA_HOME}/bin/java -classpath ${BYTEMAN_JAR}:${BYTEMAN_SUBMIT_JAR} org.jboss.byteman.agent.submit.Submit -p <PORT>  -y $label=
done
${JAVA_HOME}/bin/java  -classpath ${BYTEMAN_JAR}:${BYTEMAN_SUBMIT_JAR} org.jboss.byteman.agent.submit.Submit -p <PORT>  -u $BYTEMAN_HOME/bytemanpkg.btm
if [[ $? == 0 ]]; then
	echo "Bravo ! => operation unsubmit rules : bytemanpkg.btm  OK"
else
	echo "Shame on me ! => operation unsubmit rules : bytemanpkg.btm failed"
fi
PATH=$OLD_PATH
export PATH
